        ...
        <v-card flat class="border mt-15 d-flex flex-column">
          <v-form ref="form" class="ma-10" @submit.prevent="handleSubmit">
            ...
            <v-row v-if="trainingLoaded">
              <v-select :items="training" item-title="description" item-value="id"
                :rules="[value => !!value || 'Selecione algum exercício']" class="pa-5" v-model="exercise_id"
                variant="outlined" label="Qual exercício" placeholder="Qual exercício" />
            </v-row>

            <v-row>
              <v-text-field class="pa-3" v-model="repetitions"
                :rules="[value => value >= 1 || 'Informe a quantidade de repetições', value => Number.isInteger(Number(value)) || 'Apenas números inteiros']"
                variant="outlined" type="number" label="Repetições" placeholder="Repetições" />
              <v-text-field class="pa-3" v-model="weight" :rules="[value => value >= 1 || 'Informe o peso utilizado']"
                variant="outlined" type="number" label="Quilos" placeholder="Quilos" />
              <v-text-field class="pa-4" v-model="break_time" :rules="[value => value >= 30 || 'Mínimo 30 segundos']"
                variant="outlined" type="number" label="Pausa(em segundos)" placeholder="Pausa(em segundos)" />
                <v-select class="my-select" :items="['Segunda', 'Terça', 'Quarta', 'Quinta', 'Sexta', 'Sabado', 'Domingo']"
                variant="outlined" v-model="day"/>
            </v-row>
            

            <v-row>

              <v-textarea class="pa-5" v-model="observations" variant="outlined" type="text" label="Observações"
                placeholder="Observações" />
            </v-row>

            <v-row>
              <v-btn class="ma-2" color="orange" variant="outlined" type="submit">Cadastrar</v-btn>
            </v-row>
          </v-form>
        </v-card>
      </v-container>
